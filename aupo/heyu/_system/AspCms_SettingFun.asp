...
Sub saveSiteSetting
	...
	dim cnzzarr:cnzzarr=split(cnzz,"@")
		if ubound(cnzzarr)>0 then
			...
			dim cnzzlink:cnzzlink="http://wss.cnzz.com/user/companion/aspcms_login.php?site_id="&cnzzarr(0)&"&password="&cnzzarr(1)&""
			dim sql:sql="update aspcms_menu set menulink='"&cnzzlink&"' where MenuKey='cnzz'"
			'die(sql)
			conn.exec sql,"exe"
		end if
	end if
