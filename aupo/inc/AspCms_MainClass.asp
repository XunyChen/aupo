	...
	Public Function parseLoop(Byval str)
		...
		for each match in matches
			...
			elseif str="about" or str="type" then			
				sql="select SortType,SortID,SortURL,SortFolder,SortFileName,SortName,SortContent,GroupID,ImagePath,IcoImage,IndexImage,Exclusive, PageDesc,SortEnName from {prefix}Sort where SortStatus=1 and SortID="&lsort&""
					...
					if str="content" or str="aboutcontent" or str="news" or str="product" or str="down" or str="pic"  then							
						...
						select case fieldName
							...
							case "downurl"
								...
								nloopstr = replace(nloopstr,matchfield.value,"/down/?"&DateArray(0,i)&".html")							
							case "tag"
				...
				for i = 1 to lsize
					...
					for each matchfield in matchesfield
						...
						select case fieldName
							...
							case "downurl"
								'nloopstr = replace(nloopstr,matchfield.value,rsObj("DownURL"))	
								nloopstr = replace(nloopstr,matchfield.value,"/down/?"&rsObj("ContentID")&".html")									
							case "istop" '置顶
								nloopstr = replace(nloopstr,matchfield.value,rsObj("IsTop"))								
							case "isrecommend" '推荐
								nloopstr = replace(nloopstr,matchfield.value,rsObj("Isrecommend"))										
